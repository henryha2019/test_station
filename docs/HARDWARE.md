...
- **图像**:Wi-Fi。板子开一个 SoftAP `PogoTest-CAM`,主机连上后 `GET http://192.168.4.1/capture`
  取 JPEG(`config → vision.espcam_url`)。串口和 Wi-Fi 同时用,互不影响。
  (想让主机留在你家 Wi-Fi 上联网,把固件改成 STA 模式即可。)
